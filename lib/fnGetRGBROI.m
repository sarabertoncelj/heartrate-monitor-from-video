function RGBData = fnGetRGBROI(videoFile, bbox, bbox2Par, videoFileOut)
...
%RGBData = zeros(1000, bbox2(4)+1, bbox2(3)+1, 3);
...
B = videoFrame(bbox2(2):bbox2(2)+bbox2(4),bbox2(1):bbox2(1)+bbox2(3),:);
    A=reshape(squeeze(B(:,:,1)),[1,48*117]);
    k = find(A>0.84 & A<0.98);
    RGBData(1,1) = mean(A(k));
    A=reshape(squeeze(B(:,:,2)),[1,48*117]);
    RGBData(1,2) = mean(A(k));
    A=reshape(squeeze(B(:,:,3)),[1,48*117]);
    RGBData(1,3) = mean(A(k));
...
%     
    ...
    A=reshape(squeeze(B(:,:,1)),[1,48*117]);
    k = find(A>0.84 & A<0.98);
    RGBData(n,1) = mean(A(k));
    A=reshape(squeeze(B(:,:,2)),[1,48*117]);
    RGBData(n,2) = mean(A(k));
    A=reshape(squeeze(B(:,:,3)),[1,48*117]);
    RGBData(n,3) = mean(A(k));
